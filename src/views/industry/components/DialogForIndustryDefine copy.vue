...
export default {
  ...
  methods: {
    ...
    handleCheckAll (checked) {
      ...
      this.$refs.searchTree.setCheckedKeys(ids)
    },
    // tree 节点渲染函数
    renderContent (h, { node, data, store }) {
      const k = `category${data.rank}`
      return (
    ...
    goRight () {
      // this.selectedIds = []
      const checked2 = this.$refs.searchTree.getCheckedKeys(true)
      const checked1 = this.$refs.lazyTree.getCheckedKeys(false)
    ...
    async renderRightTree () {
      ...
      if (res.code === 200) {
        ...
        this.$nextTick(() => {
          this.$refs.lazyTree.setCheckedKeys(this.selectedIds, false)
          this.$refs.searchTree.setCheckedKeys(this.selectedIds, false)
        })
      }
    },
    // left tree
