  ...
  <el-select
    ...
    <el-option value="0" class="hidden"></el-option>
      ...
      <el-tree
        ...
        <Cate-Select-Tree-Label slot-scope="{data}" :data="data" :likeCondition="likeCondition"/>
      </el-tree>
      <!-- 异步请求树 -->
      <el-tree
        v-show="!showExpandTree"
        ...
        :load="loadNode"
        @node-click="handleNodeClick">
        <Cate-Select-Tree-Label slot-scope="{data}" :data="data" label="category"/>
      </el-tree>
  </el-select>
</template>
...
import CateSelectTreeLabel from '@/views/tmall-category/components/CateSelectTreeLabel.vue'
import RankIcon from '@/components/RankIcon.vue'
export default {
  name: 'SelectTree',
  components: { CateSelectTreeLabel, RankIcon },
  props: {
    defaultObj: {
      type: Object,
      default: () => {}
